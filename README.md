...
- **QuickBooks Online sync** — Admin configures OAuth Client ID/Secret in Settings, connects the company, then uses Data → **Sync with QBO** to pull a date range via the Transaction List report and the same account-picker import flow ([src/QboService.php](src/QboService.php)).
